<template>
	<article class="article">
		<NuxtLink :to="`/article/${article.slug}`" class="article-img-container">
			<img :src="article.featured_image" class="article-img" :alt="article.featured_image_alt">
		</NuxtLink>
		...
		<p>{{ article.summary }}</p>
		<button class="btn-secondary" @click="$router.push(`/article/${article.slug}`)">
			<span>Lire plus</span>
		</button>
	</article>
</template>
...
.article-img-container {
	width: 100%;
	box-shadow: 0 10px 25px 4px rgb(23 25 32 / 21%);
	border-radius: 15px;
	overflow: hidden;
	display: flex;
	width: 100%;
